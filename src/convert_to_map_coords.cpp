...
class ImageConverter
{
  ...
  public:
    ...
    void setOriginalX(double x) { original_x = x; }
    void setOriginalY(double y) { original_y = y; }
    ...
    void imageCb(const sensor_msgs::ImageConstPtr& image_msg)
    {
        ...
        try {
            ...
            original_coords.setValue(original_x, original_y, 0.0);
            std::cout << "original coords : " << "x : " << original_x << "  y : " << original_y << "  z : 0.0" << std::endl;
            ...
            int max_cost = 0;
            for (size_t i=top_v; i<bottom_v; i++) {
                for (size_t j=left_u; j<right_u; j++) {
                    if (int(cv_ptr->image.data[i* image_width + j]) > max_cost) {
                        max_cost = int(cv_ptr->image.data[i* image_width + j]);
                    }
                }
            }
...
int main(int argc, char** argv)
{
    ...
    ImageConverter ic;
    ic.setOriginalX(std::atof(argv[1]));
    ic.setOriginalY(std::atof(argv[2]));
    ros::spin();
    return 0;
}
